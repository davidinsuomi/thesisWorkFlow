...
public class LocalEndpoint extends Endpoint {
	...
	private static String[] getResourcePath(Request request) {
		List<Option> options = request.getOptions(OptionNumberRegistry.URI_PATH);
	...
	private static String getResourceIdentifier(Request request) {
		...
		List<Option> uriPaths = request.getOptions(OptionNumberRegistry.URI_PATH);
		
		if (uriPaths == null) return "";
		
		StringBuilder builder = new StringBuilder();
		for (Option uriPath : uriPaths) {
			builder.append('/');
			builder.append(uriPath.getStringValue());
		}
	...
	protected void printEndpointInfo(PrintStream out) {
		...
		out.println("************************************************************");
		out.println("This CoAP endpoint is using the JCoAP library");
